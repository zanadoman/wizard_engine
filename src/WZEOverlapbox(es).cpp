...
namespace wze
{
    ...
    double engine::actors::actor::overlapboxes::overlapbox::GetOffsetLength()
    {
        return this->OffsetLength;
    }

    double engine::actors::actor::overlapboxes::overlapbox::SetOffsetLength(double OffsetLength)
    {
        if (OffsetLength != OffsetLength)
        {
            printf("wze::engine.actors[].overlapboxes[].SetOffsetLength(): OffsetLength must not be NaN\nParams: OffsetLength: %lf\n", OffsetLength);
            exit(1);
        }
    ...
    double engine::actors::actor::overlapboxes::overlapbox::GetOffsetAngle()
    {
        return this->OffsetAngle;
    }

    double engine::actors::actor::overlapboxes::overlapbox::SetOffsetAngle(double OffsetAngle)
    {
        if (OffsetAngle != OffsetAngle)
        {
            printf("wze::engine.actors[].overlapboxes[].SetOffsetAngle(): OffsetAngle must not be NaN\nParams: OffsetAngle %lf\n", OffsetAngle);
            exit(1);
        }
